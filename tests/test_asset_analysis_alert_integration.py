"""Tests statiques de l'intégration des alertes dans asset_analysis."""
import ast
...
class AssetAnalysisAlertIntegrationTests(unittest.TestCase):
    ...
    def test_ordre_et_protection_locale(self):
        positions=[SEGMENT.index("afficher_plan_risque(plan_risque)"),SEGMENT.index("construire_alertes("),SEGMENT.index("afficher_alertes(alertes)")]
    ...
    def test_repli_exact_et_suite_accessible(self):
        ...
        self.assertLess(SEGMENT.index("afficher_alertes(alertes)"),SEGMENT.index('st.subheader("🤖 Analyse complémentaire par l’IA")'))
